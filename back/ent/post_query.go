...
type PostQuery struct {
	...
	withIPAddressLog *IPAddressLogQuery
...
// QueryIPAddressLog chains the current query on the "ip_address_log" edge.
func (pq *PostQuery) QueryIPAddressLog() *IPAddressLogQuery {
	query := (&IPAddressLogClient{config: pq.config}).Query()
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withIPAddressLog: pq.withIPAddressLog.Clone(),
...
// WithIPAddressLog tells the query-builder to eager-load the nodes that are connected to
// the "ip_address_log" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithIPAddressLog(opts ...func(*IPAddressLogQuery)) *PostQuery {
	query := (&IPAddressLogClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withIPAddressLog = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withIPAddressLog; query != nil {
		if err := pq.loadIPAddressLog(ctx, query, nodes,
			func(n *Post) { n.Edges.IPAddressLog = []*IPAddressLog{} },
			func(n *Post, e *IPAddressLog) { n.Edges.IPAddressLog = append(n.Edges.IPAddressLog, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadIPAddressLog(ctx context.Context, query *IPAddressLogQuery, nodes []*Post, init func(*Post), assign func(*Post, *IPAddressLog)) error {
	...
	query.withFKs = true
	query.Where(predicate.IPAddressLog(func(s *sql.Selector) {
		s.Where(sql.InValues(post.IPAddressLogColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.post_ip_address_log
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_ip_address_log" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_ip_address_log" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
